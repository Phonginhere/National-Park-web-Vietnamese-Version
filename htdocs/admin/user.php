 ...
 *
 * Trang quản lý nhân viên quản trị
 */
// cấu hình hệ thống
...
foreach ($results as $result) 
{
	...
	$users[] = array(
		...
		'edit'       => "/admin/user-edit.php?user_id=".$result['user_id'].$url
	);
}
...
$sort_username   = "/admin/user.php?sort=username".$url;
$sort_fullname   = "/admin/user.php?sort=fullname".$url;
//$sort_job_title  = "/admin/user.php?sort=job_title".$url; // bỏ đi vì cột này không nằm trên bảng user
$sort_status     = "/admin/user.php?sort=status".$url;
$sort_date_added = "/admin/user.php?sort=date_added".$url;
...
$web_title = 'Nhân Viên';
$web_content = $_SERVER["DOCUMENT_ROOT"]."/ui/admin/view/view-user-list.php";
